...
export class GithubImporter implements Importer {
  async importFromUrl(url: string): Promise<Project> {
    ...
    const readme = await this.repoReadme(url);
    ...
    if (maintainedByOasis) {
      ...
      for (let i=0; i<contributors.length; i++) {
        authors.push({[(await contributors[i]).name]: (await contributors[i]).html_url});
      }
    }

    return {
      authors,
      name: projectInfo.name.split('-').join(' ').replace(/(?:^|\s)\S/g, c => c.toUpperCase()),
  ...
  async repoReadme(url: string): Promise<string> {
    ...
    const readmeData = await await this.fetchGithub(readmeUrl);
