...
export class DoraHacksImporter implements Importer {
  async importFromUrl(projectUrl: string): Promise<Project> {
    ...
    let slug = projectData.name.toLowerCase().replace(/\s+/g, '-');
    ...
    const markdownImageMatches = projectData.project_description.match(/!\[.*\]\((.*\.(jpg|jpeg|png|JPG|JPEG|PNG))\)/g) || [];
    const markdownImages = markdownImageMatches.map(match => {
    ...
    if (projectData.hackathon_projects && Array.isArray(projectData.hackathon_projects)) {
