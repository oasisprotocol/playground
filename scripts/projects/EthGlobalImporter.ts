import {detectParatimes, detectTags, Importer} from './Importer';
...
export class EthGlobalImporter implements Importer {
  async importFromUrl(projectUrl: string): Promise<Project> {
    ...
    const h3description: HTMLHeadingElement | undefined = Array.from(document.getElementsByTagName('h3')).find(
      (el: HTMLHeadingElement): boolean => el.textContent === 'Project Description'
    );
    const projectDescription: string = h3description?.nextElementSibling?.textContent || '';
    const howItsMade: string = h3description?.nextElementSibling?.nextElementSibling?.nextElementSibling?.textContent || '';
    ...
    const codeUrlElement = Array.from(document.getElementsByTagName('a')).find(
      (el: HTMLAnchorElement): boolean => el.textContent?.includes('Source Code')
    );
    const codeUrl = codeUrlElement?.href || '';
    const maintainedByOasis = codeUrl.includes('oasisprotocol')

    const swiperSlides = Array.from(document.getElementsByClassName('swiper-slide'));
    const screenshots = swiperSlides
      .map(slide => slide.getElementsByTagName('img')[0]?.src)
      .filter(src => src);  let videoTutorial = undefined;
    ...
    let tags: string[] = detectTags(description);
    const h3createdAt: HTMLHeadingElement | undefined = Array.from(document.getElementsByTagName('h3')).find(
      (el: HTMLHeadingElement): boolean => el.textContent === 'Created At'
    );
    const hackathonName = h3createdAt?.nextElementSibling?.querySelector('div')?.textContent || '';
    tags.push(hackathonName);
    tags.push("Hackathon");

    let paratimes: string[] = detectParatimes(description);
