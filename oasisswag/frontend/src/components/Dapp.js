...
export class Dapp extends React.Component {
  ...
  async _initializeEthers() {
    ...
    this._swag = new ethers.Contract(
      contractAddress.Token,
      RandomSwagArtifact.abi,
      this._provider,
    );
    this._swagWrite = new ethers.Contract(
      contractAddress.Token,
      RandomSwagArtifact.abi,
  ...
  async _updateSwagData() {
    const swagData = await this._swag.dumpSwags();
...
//    this.setState({ swag });
  ...
  async _transferTokens(to, amount) {
    ...
    try {
      ...
      const tx = await this._swagWrite.transfer(to, amount);
  ...
  _checkNetwork() {
    if (window.ethereum.networkVersion == contractAddress.chain_id) {
      return true;
    }
